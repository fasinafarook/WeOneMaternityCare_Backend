...
class AdminController {
  ...
  async getAllServiceProviders(
    ...
    try {
      ...
      const { serviceProviders, total } =
        await this.adminCase.getAllServiceProviders(page, limit);
      return res.status(200).json({
        success: true,
  ...
  async getServiceProviderDetails(
    ...
    try {
      ...
      const serviceProvidersDetails =
        await this.adminCase.ServiceProviderDetails(id);
      return res.status(200).json({
        success: true,
  ...
  async approveServiceProviders(
    ...
    try {
      const { id } = req.params;
      const serviceProviderAppproved =
        await this.adminCase.approveServiceProvider(id);
  ...
  async addCategory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const categoryAdded = await this.adminCase.addCategory(
        categoryName,
        subCategories
      );
  ...
  async findAllCategories(req: Request, res: Response, next: NextFunction) {
    try {
      const page = req.query.page ? parseInt(req.query.page as string) : 1;
      const limit = req.query.limit ? parseInt(req.query.limit as string) : 5;
      const { categorys, total } = await this.adminCase.findAllCategories(
        page,
        limit
      );
      return res.status(200).json({
  ...
  async updateBlogStatus(
    ...
    try {
      const updatedBlog = await this.adminCase.updateBlogStatus(
        blogId,
        isListed
      );
  ...
  async toggleWebinarListing(
    ...
    try {
      ...
      return res.json({
        success: true,
        message: "Webinar listing status updated",
        isListed: webinar.isListed,
      });
    } catch (error) {
