...
type DecodedToken = {
  ...
  class ServiceProviderUseCase {
    ...
    async findServiceProvider(serviceProviderInfo: ServiceProvider) {
      const serviceProviderFound = await this.iServiceProviderRepository.findByEmail(
 ...
 async saveServiceProvider(token: string, otp: string) {
      ...
      const serviceProviderSave = await this.iServiceProviderRepository.saveServiceProvider(
        decodedToken.info
      );
      ...
      if (!serviceProviderSave) {
        ...
        const newWallet = await this.iWalletRepository.createWallet(serviceProviderSave._id as string, 'serviceProvider');
...
async serviceProviderLogin(email: string, password: string) {
 ...
 async saveServiceProviderDetails(serviceProviderDetails: ServiceProvider) {
        ...
        const serviceProvider = await this.iServiceProviderRepository.findServiceProviderById(
          _id
        );
        if (!serviceProvider) {
          throw new AppError("serviceProvider not found!", 404);
    ...
    console.log('rr',serviceProviderDetails.profilePicture,'fff',serviceProviderDetails.experienceCrt);
      ...
      async getProfileDetails(userId: string) {
        const user = await this.iServiceProviderRepository.findServiceProviderById(userId);
        return user
      }
      ...
      async addSlot(slotData: ProviderSlot) {
        ...
        if (!serviceProviderId || !slots || !Array.isArray(slots) || slots.length === 0) {
            throw new AppError("Invalid slot data", 400);
        }
        const slotAdded = await this.iServiceProviderRepository.saveProviderSlot(slotData);
        return slotAdded;
    
      }
    
    
      async getProviderSlots(serviceProviderId: string, page: number, limit: number, searchQuery: string) {
        const {slots, total} = await this.iServiceProviderRepository.getProviderSlots(serviceProviderId, page, limit, searchQuery)
        return {slots, total}
      }
      ...
      async getScheduledBookings(serviceProviderId: string, page: number, limit: number) {

        const {bookings, total} = await this.iServiceProviderRepository.getScheduledBookings(serviceProviderId, page, limit)
        return {bookings, total}
    }

    async updateWallet(serviceProviderId: string, amount: number, type: "credit" | "debit") {
    
      const wallet = await this.iWalletRepository.updateWallet(serviceProviderId, amount, type);
      return wallet
    }

    async getPaymentDashboard(serviceProviderId: string) {

      const details = await this.iServiceProviderRepository.getPaymentDashboard(serviceProviderId)
      return details
    }
