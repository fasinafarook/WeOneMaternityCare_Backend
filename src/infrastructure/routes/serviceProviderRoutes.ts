...
const serviceProviderCase = new ServiceProviderUseCase(
  serviceProviderRespository,
  otp,
  jwt,
  mail,
  hash,
...
serviceProvider.post(
  ...
  uploadStorage.single("experienceCrt"),
  (req, res, next) => {
    controller.verifyServiceProviderEmail(req, res, next);
  }
);
serviceProvider.post("/verify-otp", (req, res, next) =>
  controller.verifyOtp(req, res, next)
);
serviceProvider.post("/verify-login", (req, res, next) =>
  controller.verifyLogin(req, res, next)
);
serviceProvider.post("/resend-otp", (req, res, next) =>
...
serviceProvider.post("/logout", serviceProviderAuth, (req, res, next) =>
...
serviceProvider.get("/profile", serviceProviderAuth, (req, res, next) =>
  controller.getProfileDetails(req, res, next)
);

serviceProvider.post("/add-slot", serviceProviderAuth, (req, res, next) =>
  controller.addProviderSlot(req, res, next)
);

serviceProvider.get("/get-slots", serviceProviderAuth, (req, res, next) =>
  controller.getProviderSlots(req, res, next)
);

serviceProvider.get("/get-domains", serviceProviderAuth, (req, res, next) =>
  controller.getDomains(req, res, next)
);

serviceProvider.get(
  "/get-scheduled-bookings",
  serviceProviderAuth,
  (req, res, next) => controller.getScheduledBookings(req, res, next)
);

serviceProvider.put("/update-wallet", serviceProviderAuth, (req, res, next) =>
  controller.updateWallet(req, res, next)
);

serviceProvider.get(
  "/getPaymentDashboardDetails",
  serviceProviderAuth,
