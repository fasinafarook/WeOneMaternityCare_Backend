...
console.log('hi',paymentIntentId);
      ...
      const type = 'credit';
      const wallet = await this.walletRepository.updateWallet(serviceProviderId, price, type)
      console.log("WALLET: ", wallet)

    }

    async cancelBooking(id: string, cancellationReason: string): Promise<{ success: boolean, message?: string }> {
      // Validate inputs
      if (!id) {
    ...
    async processRefund(id: string, price: number): Promise<{ success: boolean }> {
      const booking = await ScheduledBookingModel.findById(id);
      if (!booking) {
          throw new AppError('Booking not found.', 404);
      }
  
      const { paymentIntentId, serviceProviderId, date, fromTime, toTime } = booking;
      if (!paymentIntentId) {
          throw new AppError('Payment Intent ID not found for this booking.', 400);
      }
  
      console.log(`Processing refund for payment ID: ${paymentIntentId}, Amount: ${price}`);
      
      const refund = await this.stripePayment.processRefund(paymentIntentId, price);
      if (!refund) {
      ...
      await this.walletRepository.updateWallet(booking.serviceProviderId, price, 'debit');
  
      await ProviderSlotModel.updateOne(
          { serviceProviderId, "slots.date": date, "slots.schedule.from": fromTime },
