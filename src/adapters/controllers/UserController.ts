...
class UserController {
    ...
    async verifyOtp(req: Request, res: Response, next: NextFunction) {
      try {
        ...
        if(saveUser.success){
          ...
          return res.status(200).json({success: true, token: saveUser.token, message: "OTP verified"})
        }else {
    ...
    async verifyLogin(req: Request, res: Response, next: NextFunction) {
      ...
      async getApprovedAndUnblockedProviders(req: Request, res: Response, next: NextFunction) {
        try {
    ...
    async getServiceProviderDetails(req: Request, res: Response, next: NextFunction) {
      try {
        ...
        const serviceProvidersDetails = await this.userCase.ServiceProviderDetails(id);
        return res
          .status(200)
          .json({
            success: true,
    ...
    async getProviderSlotsDetails(req: Request, res: Response, next: NextFunction) {
    try {
        const { serviceProviderId } = req.params;

        if (!serviceProviderId) throw new AppError("Service provider ID is required", 400);

        // Fetch details based on serviceProviderId
        const details = await this.userCase.getProviderSlotDetails(serviceProviderId);
    ...
    async getScheduledBookingList(req: Request, res: Response, next: NextFunction) {
      try {
        const userId = req.userId;
        ...
        const {bookings, total} = await this.userCase.getScheduledBookingList(userId, page, limit)
    ...
    async getListedWebinars(req: Request, res: Response,next: NextFunction): Promise<void> {
      try {
