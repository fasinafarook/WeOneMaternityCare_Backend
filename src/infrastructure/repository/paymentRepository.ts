...
class PaymentRepository implements IPaymentRepository {
    ...
    async bookSlot(info: any): Promise<void | null> {
        ...
        const { serviceProviderId, to, from, _id, date, userId, price, title, description, roomId,paymentIntentId } = info
      ...
      async cancelBooking(id: string, cancellationReason: string): Promise<{ success: boolean, message?: string }> {
        try {
          ...
          if (booking.status !== 'Scheduled') {
            return { success: false, message: 'Only scheduled bookings can be canceled.' };
          }
