...
interface IAdminRepository {
    ...
    findAllServiceProviders(page: number, limit: number): Promise<{serviceProviders: ServiceProvider[], total: number}>
    ...
    findAllCategories(page: number, limit: number): Promise<{categorys: Category[], total: number}>
    unlistCategory(id: string): Promise<Category | null>

    addBlog(blogData: Partial<IBlog>): Promise<IBlog>;
    listBlogs(page: number, limit: number): Promise<{ blogs: IBlog[], total: number }>;
